...
export function Insulation() {
  ...
  return (
    <section id="work" className="relative w-full h-screen overflow-hidden bg-[#1F2D3D]">
      {/* Background Image */}
      <div className="absolute inset-0">
        <img
          src="/insulation_thermal.jpg"
          alt="Insulation installation"
          className="w-full h-full object-cover"
        />
        <div className="absolute inset-0 bg-[#1F2D3D]/68" />
      </div>
      ...
      <div className="relative z-10 h-full flex flex-col justify-center px-6 lg:px-[9vw]">
        {/* Headline */}
        <h2
          className={`text-white font-bold leading-tight tracking-tight max-w-[44vw] transition-all duration-800 delay-200 ${
            isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-16'
          }`}
          style={{ fontSize: 'clamp(32px, 4vw, 56px)' }}
        >
          {t.insulation.headline}
        </h2>
